...
import conv_cnn_2_attenpooling_2 as cnn
import time,os
import numpy as np
...
def compute_accuracy(sess,Q_data,Q_placeholder,A_data,A_placeholder,L_data,L_placeholder,Keep_placehloder,logist,step):
    ...
    correct_prediction = tf.equal(tf.argmax(y_pre,1), tf.argmax(L_data,1))
    accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
    result = sess.run(accuracy, feed_dict={Q_placeholder: Q_data,A_placeholder:A_data,Keep_placehloder:1})
    print 'step %d:  acc = %.2f' %(step,result)
    file = open('./cnn_2_att_2/cnn2_'+str(step)+'_pre.txt','w')
    for y in y_pre:
        print y
        file.write(str(y[1])+'\n')
    return y_pre
...
def run_training(Q,A,L):
    with tf.Graph().as_default():
        ...
        summary_writer = tf.summary.FileWriter('./cnn_2_att_2_log/', sess.graph)
        batch = 0
