...
import conv_cnn_2_attentionpooling_2c as cnn
import time,os
import numpy as np
...
def predict(sess,Q_data,Q_placeholder,A_data,A_placeholder,Keep_placehloder,logist,step):
    ...
    file = open('./cnn_2_att_2/'+str(word2vec_dimension)+'/cnn2_'+str(step)+'_pre.txt','w')
    for y in y_pre:
        # print y[0]
        file.write(str(y[0])+'\n')
    return y_pre
...
def run_training(Q,A,L,T_q,T_a):
    ...
    with tf.Graph().as_default():
        ...
        summary_writer = tf.summary.FileWriter('./cnn_2_att_2/'+str(word2vec_dimension)+'/', sess.graph)
...
Q_data = load_data(data_path+'nlpcc2017-question-seq-all.dbqa.training-data',np.inf)
A_data = load_data(data_path+'nlpcc2017-answer-seq.dbqa.training-data',np.inf)
