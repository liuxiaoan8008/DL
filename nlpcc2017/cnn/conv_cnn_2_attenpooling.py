...
def inference(q_data,a_data,word2vec_dimension,
    ...
    with tf.name_scope('MLP_1'):
        q_pool2_flat = tf.reshape(q_h_pool2, [-1, 50 * word2vec_dimension * conv2_filter_number])
        a_pool2_flat = tf.reshape(a_h_pool2, [-1, 50 * word2vec_dimension * conv2_filter_number])
        q_add_a = tf.concat(1,[q_pool2_flat,a_pool2_flat])
        W_fc1 = weight_variable([50 * word2vec_dimension * conv2_filter_number*2, fc_1_w*conv2_filter_number])
