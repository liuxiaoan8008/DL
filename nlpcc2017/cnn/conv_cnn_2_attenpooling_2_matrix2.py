...
def inference(q_data,a_data,word2vec_dimension,
    ...
    with tf.name_scope('attention_1'):
        matrix_u_1 = weight_variable([input_length,input_length,conv1_filter_number])
        temp_1 = tf.multiply(q_h_conv1,matrix_u_1)
        tan_matrix = tf.multiply(temp_1,tf.transpose(a_h_conv1,perm=[0,2,1,3]))
        q_h_pool1_g = max_att_pooling(tan_matrix,1,100)
        tf.summary.histogram('q_conv1/q_h_pool1', q_h_pool1_g)
        a_h_pool1_g = tf.transpose(max_att_pooling(tan_matrix,100,1),perm=[0,2,1,3])
        tf.summary.histogram('q_conv1/a_h_pool1', a_h_pool1_g)
    ...
    with tf.name_scope('q_conv2'):
        ...
        q_h_conv2 = tf.nn.tanh(conv2d(q_h_pool1_g,q_W_conv2,padding='SAME') + q_b_conv2)
    ...
    with tf.name_scope('a_conv2'):
        ...
        a_h_conv2 = tf.nn.tanh(conv2d(a_h_pool1_g, a_W_conv2,padding='SAME') + a_b_conv2)

    with tf.name_scope('attention_2'):
        matrix_u_2 = weight_variable([input_length, input_length, conv1_filter_number])
        temp_2 = tf.multiply(q_h_conv2, matrix_u_2)
        tan_matrix_2 = tf.multiply(temp_2,tf.transpose(a_h_conv2,perm=[0,2,1,3]))
        q_h_pool2_g = max_att_pooling(tan_matrix_2,1,100)
        tf.summary.histogram('q_conv1/q_h_pool1', q_h_pool2_g)
        a_h_pool2_g = tf.transpose(max_att_pooling(tan_matrix_2,100,1),perm=[0,2,1,3])
        tf.summary.histogram('q_conv1/a_h_pool1', a_h_pool2_g)

    #fc1
    with tf.name_scope('MLP_1'):
        q_pool2_flat = tf.reshape(q_h_pool2_g, [-1, 100 * word2vec_dimension * conv2_filter_number])
        a_pool2_flat = tf.reshape(a_h_pool2_g, [-1, 100 * word2vec_dimension * conv2_filter_number])
        q_add_a = tf.concat(1,[q_pool2_flat,a_pool2_flat])
        W_fc1 = weight_variable([100 * word2vec_dimension * conv2_filter_number*2, fc_1_w*conv2_filter_number])
